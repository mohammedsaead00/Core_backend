...
### 2026-09-07 — Session 3 (continued) — VERIFY SCRIPT + USER FLOW DOCS
- **`verify.ps1`** (repo root): one command that builds, runs all tests, boots the real API on a scratch LocalDB DB (`Database:MigrateOnStartup=true`), mints an HS256 test JWT in PowerShell, walks the main user flows over HTTP (sign-up → onboarding → goals → nutrition + summary sync → streak → workouts + sync → chat unread → public data → 401/400/503 security guards) and prints a PASS/FAIL checklist; drops the scratch DB unless `-KeepDb`.
- **`docs/USER_FLOWS.md`**: Mermaid user journeys — onboarding, 4-path nutrition logging converging on the auto-synced summary + streak rules, workout flow, the full coaching lifecycle (directory → Stripe checkout → webhook → chat → policy-guarded monitoring → reviews), reminder/background jobs, barcode tiers.
- The script immediately paid off: a fresh `dotnet build` surfaced 3 new test files with missing `using Microsoft.EntityFrameworkCore;` that stale-binary runs had masked, plus `Ok(null)`/environment issues — all fixed. Final state: **127/127 tests, 23/23 verify checks, exit 0.**

### 2026-09-07 — Session 3 (continued) — SCHEMA ALIGNED WITH LIVE PROD DUMP
- The owner supplied the **real Supabase schema dump** (every CHECK, FK target, UNIQUE and default). Diffing it against the port found gaps the original inventory never showed. Migration `AlignWithProdSchema` (+ `SQL/006_align_with_prod_schema.sql`) applies:
  - **3 functional bugs fixed:** `messages.type` now allows `workout_plan`/`nutrition_plan` (6 values — the port would have REJECTED real plan messages); `barcode_products.source` includes `manual`; the messaging service writes `notifications.type = 'message'` (the CHECK only allows `message`|`plan` — `chat_message` would have failed every chat).
  - **Open question 12 RESOLVED:** `notifications.coach_id` references `profiles(id)` with an FK — FK added (was a deliberate no-FK soft reference). `notifications.plan_id` has NO FK in prod — the port's extra FK removed.
  - **`barcode_scan_history.barcode` is nullable with NO FK** in prod (scan may predate the cache) — port aligned.
  - **Open question 6 RESOLVED:** UNIQUE on `onboarding.user_id`, `user_goals.user_id`, `user_active_program.user_id`, `coaches.user_id`, `stripe_customers.user_id` + `stripe_customer_id` — all now unique indexes.
  - **~22 CHECK constraints added** with the now-known value lists (gender, fitness_goal, onboarding fields, meal_type, muscle groups, exercises.category, training level/goal, mood, reviews.rating, subscriptions.tier/payment_status, payment_intents.status, coach_content.type, subscription_phases.type/status, coach_profiles.rating, confidence columns, weekly_activity day_index/pcts).
  - **Type fixes:** weekly_activity actual_pct/goal_pct are INT (0–100), daily_activity heart_rate_avg/exercise_minutes are numeric — weekly_progress view updated to CAST the AVG (int AVG would truncate).
  - **Nullability/defaults aligned:** ~25 columns made NOT NULL (or given defaults) per the dump — coaches.bio/is_active, subscriptions.tier/start_date, payment_intents.currency/status/tier, coach_onboarding display_name/bio/prices/languages/max_clients/certifications/specialization, nutrition_logs.logged_date, workout_sessions.session_date/started_at, workout_sets.logged_at, daily_activity.activity_date/synced_at, food/voice scans scanned_at/logged_at, barcode confidence + history scanned_at, user_streaks.freeze_available, body_measurements.measured_date, client_assignments.assigned_at; daily_summary zero-defaults added.
